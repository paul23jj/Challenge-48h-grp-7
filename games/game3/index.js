...
function handleKeyDown(event) { // Function to handle keydown events
    ...
    if (checkVictory()) { // Check if the player has completed the level
        if (levelIndex < Levels.length - 1) {
            ...
            showModal("Élicitations !", "Le code est : 26\n\nNote bien ce mot pour la phrase finale !");
            // Sur fermeture du modal final → enregistre et retourne au hub
            document.getElementById('modalClose').onclick = function() {
                document.getElementById('customAlert').style.display = 'none';
                if (typeof window.ARG_complete === 'function') {
                    window.ARG_complete('game3', 1000, '26');
                } else {
